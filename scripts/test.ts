...
import {
  registerPointerEncoding,
  storeCodeEncoding,
  instantiateContractEncoding,
} from "./proto_encoder.js";
...
async function main() {
  try {
    ...
    const client = await SigningCosmWasmClient.connectWithSigner(
      RPC_ENDPOINT,
      wallet,
      { gasPrice: GAS_PRICE, registry: customRegistry }
    );
    ...
    const result2 = await client.signAndBroadcast(
      account.address,
      messages,
      fee
    );
